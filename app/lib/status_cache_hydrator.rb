...
class StatusCacheHydrator
  ...
  def hydrate(account_id)
    ...
    payload[:application] = REST::ApplicationSerializer.render_as_hash(@status.application, view: :public) if payload[:application].nil? && @status.account_id == account_id && @status.application.present?
    ...
    if payload[:reblog]
      ...
      payload[:reblog][:application] = REST::ApplicationSerializer.render_as_hash(@status.reblog.application, view: :public) if payload[:reblog][:application].nil? && @status.reblog.account_id == account_id && @status.reblog.application.present?
